...
The benefit of using Pandas is that it allows one to easily convert data types. This stage is called data manipulation, and it's where Pandas shines. We can use functions that are specific to the Pandas library to convert from one data type to another without altering the metadata itself. For instance, if a survey was recording the birthdays of its users to find out, on average, how many users celebrate their birthday during the holiday months. Pandas allows us to convert strings in the form of '12-24-2001' into a `datetime` object for us to easily extract month, day, or year. Common functionalities such as these are made efficient processes with the help of Pandas. 
